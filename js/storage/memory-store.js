var MemoryStore = function(successCallback, errorCallback) {
    ...
    this.gpio_findByName = function(searchKey, callback) {
        var lst = this.gpio_list.filter(function(element) {
            var fullName = element.name + " " + element.group;
            return fullName.toLowerCase().indexOf(searchKey.toLowerCase()) > -1;
        });
        callLater(callback, lst);
    }
    
    this.gpio_findById = function(id, callback) {
        var lst = this.gpio_list;
        var gpio = null;
        var l = lst.length;
        for (var i=0; i < l; i++) {
            if (lst[i].id === id) {
